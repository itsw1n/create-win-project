...
export function buildVars(answers) {
  ...
  return {
    ...
    FRONTEND_PORT: getFrontendPort(answers.stack),
    BACKEND_PORT: getBackendPort(answers.stack),
  }
}

function getFrontendPort(stack) {
  if (stack.startsWith('nextjs')) return '3000'
  return '5173'
}

function getBackendPort(stack) {
  if (stack.includes('springboot')) return '8080'
  return ''
}
