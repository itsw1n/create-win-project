  ...
  const walk = async (dir) => {
    ...
    for (const e of entries) {
      ...
      if (e.name.endsWith('.manifest.json')) {
        ...
        m._file = full
        const rel = path.relative(playbooksDir, full).replace(/\.manifest\.json$/, '.md')
        m._playbook = m.kind === 'styling' ? `styling/${m.id}-extensions.md` : rel
        manifests.push(m)
      }
    }
  }
...
export function resolveStack(answers, catalog) {
  ...
  let be = catalog.byId[answers.backend]
  if (!fe) throw new Error(`Unknown frontend: ${answers.frontend}`)
  if (!be && answers.backend === 'postgres') {
    be = { id: 'postgres', label: 'PostgreSQL (Prisma)', port: 5432, needsDocker: true }
  }
  ...
  const playbooks = selected.map((m) => m._playbook).filter(Boolean)
  let folders = []
