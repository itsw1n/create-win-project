...
export function validateManifests(manifests) {
  const ids = new Set()
  const validKinds = new Set(['frontend', 'backend', 'database', 'migration', 'styling', 'devops', 'universal'])
  for (const manifest of manifests) {
    ...
    for (const field of ['deps', 'devDeps']) {
      if (manifest[field] && (Array.isArray(manifest[field]) || typeof manifest[field] !== 'object')) {
        throw new Error(`Manifest ${manifest.id}.${field} must be an object`)
      }
    }
