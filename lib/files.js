...
RULES.md is the index — read it to find the playbook section that governs each concern.
Load the relevant playbook from /playbooks/ when working on a specific concern.

---

## This Combo — Key Constraints
${isNextjs && stack.architecture === 'large'
    ? '- Architecture: LARGE — features use `features/[name]/{components, actions, services, queries, repositories, schemas}` + types.ts'
    : isNextjs
      ? '- Architecture: MEDIUM — features use a Service layer, no Repositories (`features/[name]/{components, actions, services, schemas}` + types.ts)'
      : ''}
${constraintsBlock}
...
├── features/         → business logic by domain — see universal/folder-structure.md for the per-feature tree
├── components/
...
### Docs
\`\`\`
docs/                → api/ (overview, endpoints, errors), architecture/ (overview, database-schema, auth-flow), guides/ (setup, deployment, env-variables)
\`\`\`
...
Full rules live in RULES.md + /playbooks/ — open the relevant playbook per concern.
