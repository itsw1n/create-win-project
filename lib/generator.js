...
import { FOLDERS, STACK_META } from './constants.js'
import { resolvePlaybooks, mergePlaybooks } from './playbooks.js'
...
export async function generateProject(answers, cliRoot) {
  ...
  const meta = STACK_META[answers.stack]

  // ── 1. Create root folder ─────────────────────────────────────────────────
  await fs.ensureDir(dest)

  // ── 2. Scaffold folder structure ──────────────────────────────────────────
  await scaffoldFolders(dest, answers, vars)

  // ── 3. Generate root files ────────────────────────────────────────────────
  await generateRootFiles(dest, answers, vars)
  ...
  if (answers.githubActions) {
    await generateCI(dest, answers)
  }

  // ── 6. Copy playbooks folder into project ─────────────────────────────────
  if (await fs.pathExists(playbooksDir)) {
    await fs.copy(playbooksDir, path.join(dest, 'playbooks'))
  }

  // ── 7. Merge playbooks into RULES.md ──────────────────────────────────────
  const playbookFiles = resolvePlaybooks(answers)
  const rulesContent = await mergePlaybooks(playbookFiles, playbooksDir)
  await write(dest, 'RULES.md', rulesContent)
}

// ─── Folder scaffolding ───────────────────────────────────────────────────────

async function scaffoldFolders(dest, answers, vars) {
  const { stack } = answers
  const meta = STACK_META[stack]
  const foldersToCreate = []

  // Always: docs + github
  foldersToCreate.push(...FOLDERS.shared)
  foldersToCreate.push(...FOLDERS.github)

  // Frontend folders
  if (meta.frontend === 'nextjs') {
    foldersToCreate.push(...FOLDERS.nextjs)
  } else if (meta.frontend === 'react-vite') {
    foldersToCreate.push(...FOLDERS['react-vite'])
  }

  // Backend folders
  if (meta.backend === 'springboot') {
    const springFolders = FOLDERS.springboot.map(f =>
      f.replace(/\{\{PACKAGE_PATH\}\}/g, vars.PACKAGE_PATH)
    )
    foldersToCreate.push(...springFolders)
  }

  // Database/migration folders
  if (meta.database === 'supabase') {
    foldersToCreate.push(...FOLDERS.supabase)
  }
  if (meta.migration === 'prisma') {
    foldersToCreate.push(...FOLDERS.prisma)
  }

  // Create all folders + .gitkeep
...
async function generateRootFiles(dest, answers, vars) {
  const { stack } = answers
  ...
  await writeTemplate(dest, 'README.md', readmeMd(vars, stack), vars)
...
async function generateCI(dest, answers) {
  const { stack } = answers
  const isNextjs = stack.startsWith('nextjs')
  const hasSpringBoot = stack.includes('springboot')
  const frontendDir = isNextjs ? '' : 'frontend'

  // Frontend CI
  const frontendCI = isNextjs ? nextjsCI() : reactCI(frontendDir)
  await write(dest, `.github/workflows/ci-frontend.yml`, frontendCI)

  // Backend CI (Spring Boot only)
  if (hasSpringBoot) {
