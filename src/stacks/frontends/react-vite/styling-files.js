...
function withRoot(files) {
  return Object.fromEntries(Object.entries(files).map(([name, value]) => [`${prefix}${name}`, value]))
}

function sharedTailwindFiles(answers, stack) {
  return {
...
const buttonVariants = cva(
  ...
  {
    variants: {
      variant: { primary: 'bg-primary text-primary-foreground hover:bg-primary/90', secondary: 'bg-muted text-foreground hover:bg-muted/70' },
      size: { medium: 'h-10 px-4 text-sm', large: 'h-12 px-6 text-base' },
    },
...
`,
    'src/App.tsx': tailwindApp(answers, stack),
  }
}

function sharedModuleFiles(answers, stack) {
  return {
...
`,
    'src/styles/base.css': `body { min-height: 100vh; background: var(--color-background); color: var(--color-foreground); font-family: Arial, Helvetica, sans-serif; }
a { color: inherit; }
`,
    'src/styles.css': `@import './styles/tokens.css';
...
`,
    'src/components/layout/Container/Container.module.css': `.container { width: min(100% - 3rem, 64rem); margin-inline: auto; }
@media (min-width: 64rem) { .container { width: min(100% - 4rem, 64rem); } }
`,
...
`,
    'src/components/common/Button/Button.module.css': `.button { display: inline-flex; align-items: center; justify-content: center; border: 0; border-radius: var(--radius-control); font-weight: 600; cursor: pointer; transition: background-color 150ms ease; }
...
`,
    'src/App.module.css': `.hero { min-height: 100vh; }
...
`,
    'src/App.tsx': moduleApp(answers, stack),
  }
}

function featureImports(stack, mode) {
  if (stack.architecture === 'small') return ''
  if (stack.architecture === 'large') return "\nimport { getStarterStatus, StarterStatus } from '@/features/status'"
  const component = mode === 'tailwind'
    ? '@/features/status/components/StarterStatus'
    : '@/features/status/components/StarterStatus/StarterStatus'
  return `\nimport { StarterStatus } from '${component}'\nimport { getStarterStatus } from '@/features/status/services/getStarterStatus'`
}

function tailwindApp(answers, stack) {
...
function addButtonTest(files, answers, mode) {
  if ((answers.testing || 'basic') === 'none') return
  const folder = mode === 'tailwind' ? 'src/components/common' : 'src/components/common/Button'
  files[`${folder}/Button.test.tsx`] = `import { render, screen } from '@testing-library/react'
...
export function addReactViteStylingFiles(files, answers, stack) {
  const mode = stack.styleId === 'tailwind' ? 'tailwind' : 'css-modules'
  if (mode === 'css-modules' && stack.architecture !== 'small') {
    delete files['frontend/src/features/status/components/StarterStatus.tsx']
  }
  const localFiles = mode === 'tailwind' ? sharedTailwindFiles(answers, stack) : sharedModuleFiles(answers, stack)
  addStatusFiles(localFiles, stack, mode)
  addButtonTest(localFiles, answers, mode)
  Object.assign(files, withRoot(localFiles))
}
